...
test('versioned profile validates every user-configurable shaping bound', () => {
  ...
  const unknownProfile = { ...profile, unexpectedTuning: true }
  assert.throws(() => createAppleSpatialInputProfile(unknownProfile), /unexpectedTuning/)
})
...
test('first sample is neutral and elapsed-time smoothing is refresh-rate independent', () => {
  ...
  const run = (stepMilliseconds: number, steps: number) => {
