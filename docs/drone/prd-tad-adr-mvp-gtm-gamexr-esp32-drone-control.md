...
worktree_id: "codex/gamexr-drone-bench"
...
verification_scope: "Host-only reference implementation; physical ESP32, Wi-Fi and flight checks not executed"
---
...
This is a new follow-on to [FW-DEV-001@0.3.0](history/provenance.json),
not a claim that its heartbeat firmware now flies. This revision implements the host
interface, local bridge and separate simulated receiver. It does not implement an
onboard flight controller or a physical radio link. The parent development-tool plan
retains its own evidence and acceptance criteria.

## Identity and grounding — reference implementation

All five roles join `DRONE-RC-001@0.2.0`. Context: the user wants to extend the existing
spatial interface to a physical drone. Intent: obtain an understandable, repeatable
local control loop with minimum duplicated software. Directive: implement a separately verifiable host slice while preserving the
aircraft acceptance path. RAO: the interface/protocol maintainer implements the host
control loop and its executable checks. SVO: Maintainer implements host bench control.

[S0](history/evidence/source-snapshot.json) binds the original inspected files;
[S1](evidence/source-snapshot.json) binds the implemented candidate and specification input. GameXR
baseline is `8334355dc2c1f9bdf463ba102a63b5a3ac579d89`; the starter is
`de5d19a47956c740631b879c33242e60f10e4fd3`. The original grounding below describes that baseline. The successor host grounding
in the implementation section records the new capability; hardware claims stay open.
...
All physical E1–E7 remain **not run**. New host criteria HA1–HA4 below are separate
checks and cannot satisfy physical E1–E7. Each is an independently observable check, not an
...
### Reference implementation — independently closable host slice

All five roles consume `DRONE-RC-001@0.2.0`. The user-authorized implementation adds
F1-H, a preparatory host control loop under P-1/F1. HA1–HA4 are Must for this slice;
they do not replace the original rig, setup or flight acceptance. H0 has no T1 hardware
dependency; T2/T3 aircraft integration still does. This is the affected re-derivation
from v0.1.0, archived with a relocated parent link and [provenance](history/provenance.json) in [history](history/prd-tad-adr-mvp-gtm-gamexr-esp32-drone-control-v0.1.0.md).

| Host criterion / VCC | Given → when → then | Trace / independent check |
|---|---|---|
| HA1 `host-command-path` | Given the loopback fixture, when a user enables and changes each axis, then the separate receiver acknowledges bounded independent setpoints and no motor outputs | P-1/F1-H → C1/C2/fixture → ADR3 → Node protocol/transport tests and mobile browser test |
| HA2 `host-authority-loss` | Given active control, when freshness, ownership or input lifecycle fails, then control is inhibited and a new enable is required; invalid/replayed traffic does not renew the receiver lease | P-1/F1-H → C1/C2/fixture → ADR2/ADR3 → 20 deterministic trials per receiver fault plus real transport and browser lifecycle/stall checks |
| HA3 `host-truth-and-isolation` | Given a simulated receiver, when telemetry or game tools are used, then unavailable sensors remain explicit, accepted setpoints are separate, and game controls cannot change the pilot setpoint | P-1/F1-H → C1/C2 → ADR2/ADR3 → browser telemetry and game API tests |
| HA4 `host-repeatability` | Given Node 24.15 and installed pinned dependencies, when checks/builds and the documented startup run, then the loopback UI opens, existing contracts pass and release budgets remain enforced | P-1/F1-H → C1/C2/C4 → ADR3 → type/source, both release modes and existing/new WebKit tests; no measured physical TTV claim |
...
|---|---|---|---|
| C1: Interface captures pilot intent and displays telemetry | GameXR; upstream clamp plus separate absolute setpoints | Implemented lazy host panel; physical profile and measured aircraft telemetry pending | host dev-proven / undocumented; aircraft spec-complete |
| C2: Bridge validates and translates the selected session | `tools/drone-bridge/` in GameXR; native networking and pinned WebSocket dependency | Implemented host-only bridge, bounded admission and CRTP codec; physical transport pending | host dev-proven / undocumented; aircraft spec-complete |
...
The target aircraft topology has three runtime zones: browser memory (pilot intent, session state),
...
### Reference implementation — implemented host contracts and grounding

HA1–HA4 from PRD derive the host component VCCs. Evidence references run in the
**authoring** surface only; the final report binds command exit codes and source hashes.

| Component / derived VCC | Invocable check and recorded result | Local / delivered |
|---|---|---|
| C1 / HA1–HA3 | `npm run test:drone-browser` — 4 passed; [report](evidence/validation.json) | host dev-proven / undocumented |
| C2 + fixture / HA1–HA3 | `node --test tests/drone-protocol.test.ts tests/drone-bridge.test.ts` — 11 passed; report | host dev-proven / undocumented |
| C1/C2/C4 / HA4 | `npm run check`, `npm run check:apex`, `npm run test:webkit` — exit 0, 70 source and 9 existing browser tests; report | host dev-proven / undocumented |
| C3 / AC1–AC7 | No physical checks run | spec-complete / undocumented |

The host VCC constraint is unchanged game/native dependency contracts, no hardware
actuation, no publication and no weakening of existing tests. The existing browser
overflow check exposed a header regression; the mobile header was fixed and the
unchanged check passed. New browser assertions target the actual disabled sliders,
not the test framework's unsupported fieldset-enabled interpretation.

Host reference implementation contracts:
- I1-H: same-origin `/gamexr/drone-socket`, JSON `enable`, `disable`, or strict
  `controls` with profile/session/challenge/sequence and four finite axes. Only one
  socket owns control. No WebMCP handle, command persistence or automatic reconnect.
- I2-H: 40 ms sender cadence; 250 ms pilot lease; 300 ms telemetry age limit;
  monotonically increasing receiver samples; fresh one-use receiver challenges;
  at most eight challenges; at most four WebSocket clients, 1 KiB inbound messages,
  80 messages/s/client and bounded socket buffers. Limits are host test parameters.
- I3-H: loopback UDP carrying an HMAC-SHA256 fixture envelope; random per-run key
  passed to the child only over inherited IPC. Receiver-local 250 ms command lease
  and challenge expiry, sequence/session rejection and neutral state on expiry.
  The event loop samples expiry every 40 ms; these host timers are not real-time
  guarantees. Hardware requires onboard equivalents and measured scheduling bounds.
- I4-H: source=`simulated`, device=`simulated-esp32`, firmware=`gamexr-bench-receiver/1`,
  motorOutputs=false; accepted setpoints/sequence/sample age shown, actual attitude
  and battery are null. In-memory ring holds 1,000 events and exports only on request;
  no stored sessions, upload, cloud or runtime model calls.
- I5-H: original 16-byte RPYT/CRTP encoder: header 0x3c, three little-endian float32
  values, uint16 thrust, sum8 checksum. Bench ranges are ±10° roll/pitch, ±45°/s yaw,
  0–10,000 thrust units. These signs are fixture conventions, not approved aircraft
  signs. The upstream legacy receiver negates yaw; calibrate the actual profile first.

Host topology: browser memory ↔ loopback HTTP/WebSocket bridge ↔ loopback UDP receiver
process. It exercises transport and admission; it contains no flight dynamics, IMU,
radio, estimator, mixer or actuator. The simulator remains the existing game mode.
Native phone networking, physical gamepad, motion/XR control, real Wi-Fi loss trials,
firmware flashing and flight remain explicit follow-ons.

| New grounding claim | Disposition | Evidence / consequence |
|---|---|---|
| Separate host command and receiver paths exist | Confirmed in candidate | `src/drone/`, `tools/drone-bridge/`, Node and browser tests, S1 |
| Shared spatial algorithms are forked for the drone | Contradicted | Reuse pinned clamp; only bench dead-zone/range policy is local; no new sensor/filter/flight algorithm |
| Browser/transport source contains a physical actuator route | Absent in inspected scope | Fixed loopback bind, spawned fixture only, CLI rejects all options except port; no hardware target |
| Wire codec is a complete stock flight-control client | Contradicted | Fixture envelope, missing board signs/arming/telemetry integration; no stock-firmware compatibility claim |
| Existing source and browser contracts still pass | Confirmed only for recorded candidate | S1 plus executable report; native Apple matrix not rerun because its code/pins are unchanged |
| Actual aircraft and product demand are validated | Unverified | E1–E7 and market evidence remain absent |
...
Status: accepted for the host slice; aircraft integration pending. Decision: GameXR owns a distinct Drone mode and a small local
...
### ADR3 — reference implementation: isolate a motor-disabled host fixture

Status: accepted for `DRONE-RC-001@0.2.0`. The user requested implementation while the
board contract is unresolved. Implement HA1–HA4 in PRD without changing AC1–AC7 or
opening B1/B2. The fixture is a separate local process reached through real UDP;
it owns a monotonic watchdog independently of the page and bridge.

A pure in-page mock is cheaper but cannot exercise UDP or a separate receiver watchdog.
A generic physical-target switch would require an unreviewed board contract. The
selected fixture has no GPIO, serial, motors, arbitrary destination or hardware mode.
Its authenticated freshness envelope is **not supported by stock ESP-Drone**. A packet
layout match alone cannot justify forwarding these commands to an aircraft.

The original CRTP codec is derived from public protocol facts, with no copied flight
implementation. Host code remains under this repository's MIT license; upstream
ESP-Drone stays GPL-3.0. A future firmware adaptation retains its own license and SDK.
Recovery is process shutdown and fresh explicit enable; no physical recovery claim.

## MVP

### Reference implementation — joined slice and roadmap

Consumes AC1–AC6/C1–C4/ADR1–ADR2 at `DRONE-RC-001@0.2.0` for the first bench slice.
...
### Reference implementation — host execution and demonstration

| Step / RAO / SVO | Joined input and output | Check / budget / status |
|---|---|---|
| H0: Interface/protocol maintainer implements host control | HA1–HA4/C1/C2/ADR2–ADR3 at exact revision above → panel, bridge, fixture and documentation | Host checks in evidence; 45 active minutes target, re-estimate on material failure; local candidate only |
| T1–T5: Named owners complete aircraft acceptance | AC1–AC7/C1–C4/ADR1–ADR2 at exact revision above | Unchanged physical exits; awaiting identified rig; no promotion from host evidence |

Host warm demo: open local URL → Drone → Connect receiver → Enable bench control →
change roll and observe accepted setpoints → Disable and export. No claim that a
physical drone setup meets the original ≤20-minute target. The source-owned
[runbook](../DRONE-CONTROL.md) gives setup, tests, limits and next hardware inputs.
...
The host slice is grounded in S1 and verified by the executable checks recorded in
[the evidence report](evidence/validation.json). Physical E1–E7 remain not run. The
overall aircraft outcome stays spec-complete locally and undocumented for delivery;
host components are dev-proven only in the stated environment. A mobile browser
emulation is not a named physical phone test. Independent full guideline conformance
and hardware-specific baseline remain open.

Before physical integration, obtain board/model/revision, IMU and other sensors,
motor/ESC interface and intended pilot device. Verify the chosen firmware before changing its SDK.
...
### 2026-09-14 — reference implementation revision record
